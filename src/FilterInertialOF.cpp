...
FilterInertialOF::FilterInertialOF(Manager* pManager,const char* pFilename): FilterBase(){
	pManager_ = pManager;
...
FilterInertialOF::~FilterInertialOF(){
}

void FilterInertialOF::update(const double& t){
...
void FilterInertialOF::filterState(InternState& x,const double& tEnd){
...
void FilterInertialOF::predictState(InternState& x, const double& tPre, const ImuMeas& m){
	double dt = tPre-x.t_;
...
void FilterInertialOF::encUpdateState(InternState& x, const EncMeas& m){
...
void FilterInertialOF::outlierDetection(InternState& x,const Eigen::Matrix<double,12,12>& Pyinv){
...
void FilterInertialOF::loadParam(const char* pFilename){
	// Open parameter file
...
void FilterInertialOF::logState(){
...
std::string FilterInertialOF::getKeyString(){
